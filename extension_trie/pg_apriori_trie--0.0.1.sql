\echo Use "CREATE EXTENSION pg_apriori_trie" to load this file. \quit

CREATE TYPE apriori_trie_type AS (support_table VARCHAR, rules_table VARCHAR);
CREATE OR REPLACE FUNCTION apriori_trie(IN json_data VARCHAR) RETURNS SETOF apriori_trie_type AS
$$
    ...
    def create_tmp_support_table(result_data, transactions_num):
        ...
        result_table_name = "pg_apriori_trie_support_" + dt_string
    ...
    def create_tmp_rule_table(result_data):
        dt_string = datetime.now().strftime("%Y%m%d%H%M%S")
        result_table_name = "pg_apriori_trie_rules_" + dt_string
        create_table_query = "CREATE TABLE " + result_table_name + \
                             "(" + \
                             "items_from VARCHAR []," + \
                             "items_to VARCHAR []," + \
